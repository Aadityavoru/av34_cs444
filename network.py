...
class Anchors(nn.Module):
    def __init__(
        ...
        for size in sizes:
            for aspect_ratio in aspect_ratios:
                ...
                # Compute the offsets (x_center, y_center, width, height)
                self.anchor_offsets.append([-w / 2, -h / 2, w / 2, h / 2])

        self.anchor_offsets = torch.tensor(self.anchor_offsets).float()
    ...
    def forward(self, x):
        ...
        y = torch.arange(0, H, dtype=torch.float32, device=x.device) * self.stride
        x = torch.arange(0, W, dtype=torch.float32, device=x.device) * self.stride
        x_centers, y_centers = torch.meshgrid(x, y, indexing='xy')

            # Prepare the anchor boxes
        anchors = []
        for offset in self.anchor_offsets:
            # Add offsets to center points
            ...
            # Concatenate anchors for all aspect ratios and sizes
        anchors = torch.cat(anchors, dim=0)  # Shape: (A, H, W, 4)

            # Reshape and repeat for batch size
        anchors = anchors.view(1, -1, H, W)  # Shape: (1, A*4, H, W)
        anchors = anchors.repeat(B, 1, 1, 1)  # Shape: (B, A*4, H, W)
